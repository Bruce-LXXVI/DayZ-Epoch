...
if (PermDialogSelected > -1) then {
	...
	[_building,true,true] execVM format["%1\tools\AdminBuild\adminBuild.sqf", EAT_directory];
};
...
if (TempDialogSelected > -1) then {
	...
	[_building,true,false] execVM format["%1\tools\AdminBuild\adminBuild.sqf", EAT_directory];
};
